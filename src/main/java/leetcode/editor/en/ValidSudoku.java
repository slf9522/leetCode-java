...
public class ValidSudoku {
    ...
    class Solution {
        public boolean isValidSudoku(char[][] board) {
            Map<Character, List<Integer>> iMap = new HashMap<>();
            Map<Character, List<Integer>> jMap = new HashMap<>();
            Map<Character, List<String>> subMap = new HashMap<>();
            for (int i = 0; i < board.length; i++) {
                for (int j = 0; j < board[i].length; j++) {
                    char digit = board[i][j];
                    if (digit == '.') continue;
                    iMap.computeIfAbsent(digit, k -> new ArrayList<>());
                    if (iMap.get(digit).contains(i)) return false;

                    jMap.computeIfAbsent(digit, k -> new ArrayList<>());
                    if (jMap.get(digit).contains(j)) return false;

                    int px = i - i % 3 + 1;
                    int py = j - j % 3 + 1;
                    String pxy = px + "," + py;
                    subMap.computeIfAbsent(digit, k -> new ArrayList<>());
                    if (subMap.get(digit).contains(pxy)) return false;

                    iMap.get(digit).add(i);
                    jMap.get(digit).add(j);
                    subMap.get(digit).add(pxy);
                }
            }
            return true;
